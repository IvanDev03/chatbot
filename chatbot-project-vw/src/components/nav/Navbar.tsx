...
export const Navbar = () => {
    ...
    const { conversation, deleteConv } = useContext(ChatContext)
        ...
        <aside className={`py-2 border shadow-xl burguer-${navState ? 'opened' : 'closed'} flex flex-col justify-between`} id='tab-aside'>
            <section>
                <img
                    ...
                    alt="Burguer icon"
                />
                <section onClick={() => handleConversation()}
                className={`m-4 flex flex-row cursor-pointer rounded-xl border-2 p-2 justify-center ${navState ? 'justify-between items-center border-stone-50' : 'rot-3 py-2'} `}>
